...
def calc_fuzzy(data,centroid,points_list):
    fuzzy_den = []
    fuzzy_df = {}
    fuzzy_df2 = {}
    counter = -1
    for cl in range(len(points_list)):
        counter+=1
        fuzzy_dist=[]
        fuzzy_den = []
        for i in data:
            fuzzy_dist.append(compute_distance(i,centroid[counter],'manhattan'))
            fuzzy_df[counter] = fuzzy_dist
        for j in range(30):
            fuzzy_den.append(np.abs(centroid[cl][j]-points_list[cl][:,j]))
            fuzzy_df2[counter] = np.max(fuzzy_den)
    return fuzzy_df, fuzzy_df2